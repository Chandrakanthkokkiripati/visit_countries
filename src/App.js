...
class App extends Component {
  ...
  deleteVisitCountry = id => {
    const {countriesList} = this.state
    const filteredCountries = countriesList.map(m => {
      if (m.id === id) {
        return {...m, isVisited: !m.isVisited}
      }
      return m
    })
  ...
  render() {
    ...
    const noCountryVisitedView = updateList.length > 0

    return (
      <div className="countries-visit-bg-container">
        <h1>Countries</h1>
        <ul className="countries-list">
          {countriesList.map(eachCountry => (
            <VisitCountries
              key={eachCountry.id}
              addCountriesList={this.addCountriesList}
              country={eachCountry}
            />
          ))}
        </ul>
        <h1>Visited Countries</h1>
        {noCountryVisitedView ? (
          <ul className="visited-countries-list-container">
            {updateList.map(eachCountry => (
              <VisitedCountries
                key={eachCountry.id}
          ...
          <div className="no-country-visited-view">
